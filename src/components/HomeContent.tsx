...
export default function HomeContent({ currentLang = 'en' }: { currentLang?: string }) {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            ...
            <div className="flex justify-center space-x-4">
              <Link href={`/${currentLang}/create`} className="px-8 py-3 bg-blue-600 text-white rounded-full font-medium hover:bg-blue-700 transition-colors shadow-md">
      ...
      <section className="w-full py-16 px-4 bg-gray-50">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-center text-gray-900 mb-4 sm:mb-6 break-words hyphens-auto">
            <TranslatedText translationKey="home.styles.title" fallback="Choose Your Style" />
          </h2>
          <p className="text-base sm:text-xl text-center text-gray-600 max-w-3xl mx-auto mb-8 sm:mb-12">
            <TranslatedText translationKey="home.styles.subtitle" fallback="Select from multiple artistic styles to create the perfect coloring page" />
          </p>
              ...
              <div key={index} className="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-shadow cursor-pointer">
                ...
                <div className="p-3 text-center border-t border-gray-100">
                  <h3 className="text-sm sm:text-base font-medium text-gray-800 break-words">
                    <TranslatedText translationKey={style} fallback={style.split('.').pop()} />
                  </h3>
                </div>
              </div>
            ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {testimonials.map((testimonial, index) => (
              <div key={index} className="bg-white rounded-xl overflow-hidden shadow-md p-6 border border-gray-100">
                <p className="text-sm sm:text-base text-gray-600 mb-4">"<TranslatedText translationKey={testimonial.text} fallback={testimonial.text} />"</p>
                <p className="text-sm sm:text-base font-medium text-gray-800">- <TranslatedText translationKey={testimonial.author} fallback={testimonial.author} /></p>
              </div>
            ))}
          </div>

          <div className="text-center mt-12">
            <Link href={`/${currentLang}/create`} className="px-8 py-3 bg-blue-600 text-white rounded-full font-medium hover:bg-blue-700 transition-colors shadow-md inline-block">
      ...
      <section className="w-full py-16 px-4 bg-blue-50">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-center text-gray-900 mb-8 sm:mb-12 break-words hyphens-auto">
            <TranslatedText translationKey="home.features.title" fallback="Why Choose Our AI Coloring Page Generator?" />
          </h2>
            ...
            {[
              ...
              <div key={index} className="bg-white rounded-xl p-4 sm:p-6 shadow-sm">
                ...
                <h3 className="text-base sm:text-lg font-bold mb-1 sm:mb-2 text-gray-800 break-words">
                  <TranslatedText translationKey={feature.title} fallback={feature.title.split('.').pop()} />
                </h3>
                <p className="text-xs sm:text-sm text-gray-600 break-words">
                  <TranslatedText translationKey={feature.desc} fallback={feature.desc.split('.').pop()} />
                </p>
              </div>
            ))}
          </div>
      ...
      <section className="w-full py-16 px-4 bg-gradient-to-r from-blue-600 to-blue-700 text-white">
        <div className="max-w-3xl mx-auto text-center">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-6">
            <TranslatedText translationKey="home.cta.title" fallback="Ready to Create Your Coloring Pages?" />
          </h2>
          <p className="text-lg md:text-xl mb-8 opacity-90">
            <TranslatedText translationKey="home.cta.subtitle" fallback="Start creating custom coloring pages in seconds - no design skills required!" />
          </p>
          <Link href={`/${currentLang}/create`} className="px-8 py-4 bg-white text-blue-600 rounded-full font-bold text-lg hover:bg-blue-50 transition-colors shadow-lg inline-block">
            <TranslatedText translationKey="home.cta.button" fallback="Get Started - It's Free!" />
          </Link>
        </div>
      </section>
    </main>
